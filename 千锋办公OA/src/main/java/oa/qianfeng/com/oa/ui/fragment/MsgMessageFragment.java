 ...
 */
public class MsgMessageFragment extends BaseNetFragment {

    @BindView(R.id.erv)
    EmptyRecyclerView erv;
    @BindView(R.id.emptyView)
    EmptyView emptyView;
    RBaseAdapter<MsgMessageBean> adapter;

    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        QFApp.getInstence().getMsgBus().register(this);
        adapter = new RBaseAdapter<MsgMessageBean>(getActivity(), android.R.layout.simple_list_item_1) {
            @Override
        ...
        adapter.setDatas(parent.getMsgData());
    }
