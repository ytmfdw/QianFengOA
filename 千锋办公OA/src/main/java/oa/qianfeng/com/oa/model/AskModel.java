...
public class AskModel implements IAskModel {
    ...
    @Override
    public void loadEditData(final LeaveBean bean, final OnGetDataListener<Map<String, Object>> listener) {
        Retrofit retrofit = new Retrofit.Builder()
                .baseUrl(API.URL_BASE)
                .addConverterFactory(ScalarsConverterFactory.create())
                .client(QFApp.getInstence().getHttpClient())
                .build();
        AskService as = retrofit.create(AskService.class);
        String url = API.URL_ASK_EDIT + "user_id=" + bean + "&ID=" + bean.id;
        Call<String> call = as.editAdk(url);
...
//                bean.department = content.get(1).getElementsByClass("middle").first().text();
    ...
    @Override
    public void postEditData(LeaveBean bean, final OnGetDataListener<String> listener) {
        ...
        AskService as = retrofit.create(AskService.class);
        String url = API.URL_ASK_EDIT + "user_id=" + bean + "&ID=" + bean.id;
        Call<String> call = as.postEditAsk(url,
                bean.id,
                bean.group_id,
                QFApp.getInstence().getLeaderMap().get(bean.boss),
                "",
